// pages/GiftingCategoryPage.jsx
import React, { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import axios from "axios";
import "./CategoryPage.css";

function GiftingCategoryPage() {
  ...
  const subcategories = ["Seasonal", "Bundle"];
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://localhost:5000/api/products/category/Gifting");
      setProducts(res.data.products || []);
    } catch (err) {
      console.error("Error fetching Gifting products:", err);
      setProducts([]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="category-page">
      ...
      <section className="products-section">
        <h2 className="section-title">
          {activeSubcategory ? `${activeSubcategory}` : "All Gift Sets"}
        </h2>
